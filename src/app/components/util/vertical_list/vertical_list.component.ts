...
@Component({
  ...
  templateUrl: './vertical_list.component.html',
...
export class VerticalListComponent implements OnInit {
  ...
  updateValid(type,data,valid){
    ...
    this.editCache[data].data[type]= !valid
    this.updatedData.emit(this.editCache[data].data);
    switch(type){
      default:{    
        this.selectedIndex.emit(data);
        this.tableSettings.Action.Valid.function() 
      }
    }
  }
  ...
  deleteButton(data){
    this.selectedIndex.emit(data);
    this.tableSettings.Action.Delete.function()     
  }
  viewButton(data){
    this.selectedIndex.emit(data);
